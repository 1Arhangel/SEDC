...
namespace MusicApp
{
    class Program
    {
        static void LongestName(List<Artist> param)
        {
            ...
            // new Method
            Console.WriteLine("The longest band name is " + 
                param.FirstOrDefault(x => x.ArtistName ==
                    param.Select(y => y.ArtistName).OrderByDescending(z => z.Length).First()).ArtistName);
        ...
        static void FirstAlbumReleased(List<Artist> param)
        {
            ...
            param.ForEach(x => albums.AddRange(x.Albums));

            
            var darko = param.FirstOrDefault(x => x.Albums.Contains(
                albums.OrderBy(y=>y.ReleaseDate).First()
                ));
            var firstAlbum = darko.Albums.Select(x => x.ReleaseDate.Year).Min();
            Console.WriteLine(darko.ArtistName + "" + firstAlbum);
        ...
        {
            var listOfArtists = InitArtists();

            var something = listOfArtists.GetCompleteList(x => x.Albums).GetCompleteList(x=>x.Songs);

            var iuh = listOfArtists.First().Albums.First().Artist.ArtistName;
